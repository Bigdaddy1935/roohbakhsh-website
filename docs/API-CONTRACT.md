...
اگر ایمیل وجود داشته باشد و هنوز تأیید نشده باشد، کد تأیید جدید ارسال می‌شود. همیشه پاسخ یکسان برمی‌گرداند (جلوگیری از user enumeration).
...
ارسال ایمیل (بازیابی رمز + تأیید ایمیل) از طریق `MailService` در `apps/api/src/modules/mail/mail.service.ts` انجام می‌شود. ترتیب انتخاب provider:

1. اگر `RESEND_API_KEY` تنظیم شده باشد → **Resend** (توصیه‌شده).
2. وگرنه اگر `SMTP_HOST` تنظیم شده باشد → **SMTP** (`nodemailer`).
3. وگرنه → فقط **لاگ سرور** (توسعه‌ی لوکال).

- اگر نه `RESEND_API_KEY` و نه `SMTP_HOST` تنظیم نشده باشد (مقدار پیش‌فرض توسعه)، هیچ ایمیلی واقعاً ارسال نمی‌شود — متن ایمیل (شامل کد تأیید) فقط در **لاگ سرور** چاپ می‌شود. این یعنی روی لوکال بدون سرویس ایمیل واقعی می‌توانید کد را از لاگ کپی کنید.
- برای ارسال واقعی با Resend: `RESEND_API_KEY` را از داشبورد resend.com بگیرید و `MAIL_FROM` را روی یک فرستنده‌ی تأییدشده تنظیم کنید.
