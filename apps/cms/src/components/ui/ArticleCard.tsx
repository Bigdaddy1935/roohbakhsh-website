...
export default function ArticleCard({ article }: { article: ArticleCardData }) {
  return (
    <div className="group/blog flex flex-col gap-y-3 sm:gap-y-4 h-full rounded-md bg-white border border-gray-100">
      <Link href={article.href} className="block">
        <img
          src={article.image}
          alt={article.title}
          loading="lazy"
          className="block w-full object-cover aspect-video rounded-md group-hover/blog:brightness-110 transition-all"
        />
      </Link>
      ...
      <div className="flex flex-col gap-y-5 justify-between h-full px-3 sm:px-4 pb-10 sm:pb-12 cursor-default relative">
        ...
        <Link
          href={article.href}
          className="absolute -bottom-5 inset-x-3 sm:inset-x-4 flex items-center justify-center gap-x-2 h-10 sm:h-11 rounded-md bg-[var(--brand)] text-white text-[13px] font-bold hover:opacity-90 transition-opacity"
        >
          مشاهده مقاله
