...
export default function VideoPlayer({ url, poster, onPlay }: { url: string; poster?: string; onPlay?: () => void }) {
  ...
  return (
    <div
      ...
      className="relative w-full bg-black aspect-video select-none rounded-md overflow-hidden"
    >
      <video
        ref={videoRef}
        src={url}
      ...
      {!playing && showControls && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="size-16 rounded-md bg-white/15 backdrop-blur-sm flex items-center justify-center border border-white/20">
            <RiPlayLargeLine size={30} className="text-white ms-1" />
          </div>
        </div>
      )}
      ...
      <div className={`absolute inset-x-0 bottom-0 px-4 pb-3 pt-8 pointer-events-none transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}>
        <div dir="ltr" className="relative h-1 mb-3 group/bar pointer-events-auto cursor-pointer">
          ...
          <div className="absolute top-1/2 -translate-y-1/2 size-3 rounded-full bg-white opacity-0 group-hover/bar:opacity-100 transition-opacity pointer-events-none" style={{ left: `calc(${progress * 100}% - 6px)` }} />
        </div>
        ...
        <div className="flex items-center justify-between gap-x-4 pointer-events-auto">
          <div className="flex items-center gap-x-3">
            <button onClick={togglePlay} className="text-white hover:text-[var(--brand)] transition-colors">
              {playing ? <RiPauseLargeLine size={22} /> : <RiPlayLargeLine size={22} />}
            </button>
            <div className="flex items-center gap-x-2 group/vol">
              ...
              <input type="range" min={0} max={1} step={0.02} value={muted ? 0 : volume} onChange={handleVolumeChange} className="w-0 group-hover/vol:w-16 transition-all duration-300 accent-[var(--brand)] cursor-pointer overflow-hidden" />
            </div>
            <span className="text-white/80 text-xs font-mono tabular-nums">{fmtTime(currentTime)} / {fmtTime(duration)}</span>
          </div>
          <div className="flex items-center gap-x-3">
            <div className="relative">
              ...
              {showSettings && (
                <div className="absolute bottom-7 end-0 bg-[#1a1a2e]/95 backdrop-blur rounded-md border border-white/10 py-2 min-w-[130px] z-10" onClick={(e) => e.stopPropagation()}>
                  <p className="text-white/50 text-[11px] font-semibold px-3 pb-1.5 border-b border-white/10 mb-1">سرعت پخش</p>
