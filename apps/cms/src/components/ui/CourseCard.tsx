...
export default function CourseCard({ course }: { course: CourseCardData }) {
  ...
  return (
    <div className="group/course flex flex-col bg-white rounded-md border border-gray-100 min-h-[402px] h-full">
      <Link href={course.href} className="block relative">
        <img
          src={course.image}
          alt={course.title}
          className="w-full aspect-video object-cover rounded-md group-hover/course:brightness-110 transition-all"
          loading="lazy"
        />
        {course.discount && (
          <span className="absolute top-2.5 end-2.5 flex items-center gap-x-0.5 px-2.5 py-1 bg-[var(--cta)] text-white text-xs font-extrabold rounded-md tracking-wide">
