...
// Mock courses per category — will be replaced by CMS data
...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="container h-full flex items-center gap-x-6">
          ...
          <nav className="hidden md:flex items-center gap-x-6 lg:gap-x-8 text-[14px] font-semibold text-[var(--ink)]">
            ...
            <div className="group relative">
              <Link
                href="/courses"
                className="flex items-center gap-x-1 whitespace-nowrap py-1 transition-colors hover:text-[var(--brand)] group-hover:text-[var(--brand)]"
              >
                {t("courses")}
                <RiArrowDownSLine
                  size={18}
              ...
              {/* Mega menu — right-aligned for RTL */}
              <div className="absolute top-full right-0 invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-150 pt-4 z-50">
                <div className="flex bg-white border border-gray-200 rounded-2xl shadow-xl shadow-black/[0.08] overflow-hidden">
                    ...
                    {CATEGORIES.map(({ id, href, Icon }) => (
                      <Link
                        ...
                        className={`flex items-center justify-between rounded-xl px-3 py-2.5 transition-colors ${
                          activeCategory === id ? "bg-gray-100" : "hover:bg-gray-50"
                        }`}
                      >
                        <span className="flex items-center gap-x-2.5">
                          <Icon
                            size={18}
                            className={`shrink-0 ${activeCategory === id ? "text-[var(--brand)]" : "text-gray-400"}`}
                          />
                  ...
                  {/* Sub-panel: mock courses */}
                  <div className="flex flex-col justify-between bg-gray-50 w-56 p-4 border-s border-gray-100">
                    <div className="flex flex-col gap-y-2.5">
                      {mockCourses.map((course) => (
                        <Link
                          key={course}
                          href={`/courses/${activeCategory}`}
                          className="text-[13px] text-[var(--ink)] hover:text-[var(--brand)] transition-colors"
                        >
                          {course}
                        </Link>
                      ))}
                    ...
                    <Link
                      href={`/courses/${activeCategory}`}
                      className="flex items-center gap-x-1 text-[12px] font-medium text-[var(--brand)] hover:underline mt-4"
                    >
                      {t("viewAll")}
            ...
            <Link href="/articles" className="whitespace-nowrap py-1 transition-colors hover:text-[var(--brand)]">
              {t("articles")}
            </Link>
            <Link href="/about" className="whitespace-nowrap py-1 transition-colors hover:text-[var(--brand)]">
              {t("about")}
            </Link>
          </nav>
          ...
          <div className="hidden md:flex items-center gap-x-4">
            ...
            {locale !== "ar" && (
              <button
                ...
                className="hidden md:flex items-center gap-x-1.5 text-[12px] font-medium px-2.5 py-1.5 rounded-lg border border-gray-300 hover:border-[var(--brand)] hover:text-[var(--brand)] transition-colors cursor-pointer"
              >
            ...
            {locale !== "ur" && (
              <button
                ...
                className="hidden md:flex items-center gap-x-1.5 text-[12px] font-medium px-2 py-1.5 rounded-lg border border-gray-300 hover:border-[var(--brand)] hover:text-[var(--brand)] transition-colors cursor-pointer"
              >
                <span className="text-base leading-none">🇵🇰</span>
                اردو
              </button>
      ...
      {/* Drawer panel — slides in from start (right in RTL) */}
      <div
        className={`fixed top-0 bottom-0 start-0 w-72 bg-[#F7F7F7] z-[70] overflow-y-auto transition-transform duration-300 ${
      ...
      >
        ...
        <div className="flex items-center justify-between h-[100px] bg-white px-5 border-b border-gray-200">
          ...
          <button
            type="button"
            onClick={() => setDrawerOpen(false)}
            className="text-gray-400 hover:text-[var(--ink)] transition-colors cursor-pointer"
          >
            <RiCloseLine size={24} />
          </button>
        </div>
        ...
        <div className="px-4 py-5 flex flex-col gap-y-5">
          ...
          <div className="flex gap-x-2">
            <Link href="/auth/signup" onClick={() => setDrawerOpen(false)} className="flex-1">
              <button
                type="button"
                className="w-full h-10 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity cursor-pointer"
            ...
            <Link href="/auth/signin" onClick={() => setDrawerOpen(false)} className="flex-1">
              <button
                type="button"
                className="w-full h-10 rounded-xl text-sm font-semibold border transition-colors cursor-pointer"
          ...
          <div className="grid grid-cols-2 gap-x-2">
            {(["ar", "ur"] as const).map((lng) => (
              <button
                ...
                className={`text-sm py-2 rounded-xl border font-medium transition-colors cursor-pointer ${
                  locale === lng
                    ? "border-[var(--brand)] text-[var(--brand)] bg-[var(--brand)]/5"
                    : "border-gray-300 text-[var(--ink)]"
                }`}
              >
                {lng === "ar" ? "العربية" : "اردو"}
              </button>
          ...
          {/* Categories */}
          <div>
            <p className="text-xs font-bold text-[var(--brand)] mb-2.5">
              {t("categories_title")}
            </p>
