...
export default function SignUpForm() {
  ...
  const fields = [
    { icon: <RiUserLine size={17} />, placeholder: ui.name, type: "text", name: "name" },
    { icon: <RiPhoneLine size={17} />, placeholder: ui.phone, type: "tel", name: "phone" },
    { icon: <RiMailLine size={17} />, placeholder: ui.email, type: "email", name: "email" },
  ];
  ...
  return (
    <AuthCard locale={locale}>
      ...
      <p className="text-sm text-gray-400 mb-6">
        {ui.sub}{" "}
        <Link href="/auth/signin" className="text-[var(--brand)] font-semibold hover:underline">{ui.subLink}</Link>
      </p>

      <form className="flex flex-col gap-y-3" onSubmit={(e) => e.preventDefault()}>
        {fields.map((f) => (
          <div key={f.name} className="relative">
            <span className="absolute start-3.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none">{f.icon}</span>
