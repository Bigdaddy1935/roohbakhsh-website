...
import {
  ...
  RiLockPasswordLine, RiCheckLine,
} from "react-icons/ri";
...
export default function SignUpForm() {
  ...
  const inputCls = (key: keyof Fields) =>
    `w-full h-11 rounded-lg border ps-4 pe-10 text-sm text-[var(--ink)] placeholder:text-gray-400 outline-none transition-colors bg-white disabled:opacity-60 ${
  ...
  return (
    <AuthCard>
      ...
      <form className="flex flex-col gap-y-3" onSubmit={handleSubmit} noValidate>
        ...
        <div>
          <div className="relative">
            ...
            <input
              ...
              dir="ltr"
              className={`${inputCls("password")} pe-16`}
            />
            <button
              type="button"
              onClick={() => setShowPass((s) => !s)}
              className="absolute end-9 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"
            >
        ...
        <div>
          <div className="relative">
            <span className="absolute end-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
              {fields.confirm && fields.confirm === fields.password
                ? <RiCheckLine size={17} className="text-emerald-500" />
                : <RiLockPasswordLine size={17} className="text-gray-400" />}
            </span>
            <input
              ...
              autoComplete="new-password"
              dir="ltr"
              className={`${inputCls("confirm")} pe-16`}
            />
            <button
              type="button"
